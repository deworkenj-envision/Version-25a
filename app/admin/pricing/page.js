...
export default function AdminPricingPage() {
  ...
  async function handleDuplicateSet(row) {
    const confirmed = window.confirm(
      `Duplicate ALL rows for:\n\n${row.product_name} / ${row.size}?\n\nThis will copy every matching row and assign the next sort numbers.`
    );

    if (!confirmed) return;

    try {
      setDuplicatingSetId(row.id);
      setMessage("Duplicating rows...");

      const matchingRows = rows
        .filter(
          (item) =>
            item.product_name === row.product_name && item.size === row.size
        )
      ...
      const productRows = rows.filter(
        (item) => item.product_name === row.product_name
      );

      const maxSort =
        Math.max(...productRows.map((item) => Number(item.sort_order || 0))) ||
        0;
      ...
      setMessage(
        `Duplicated ${matchingRows.length} row(s) for ${row.product_name} / ${row.size}.`
      );
      await loadPricing();
    } catch (err) {
      console.error(err);
      setMessage(err.message || "Duplicate failed.");
    } finally {
