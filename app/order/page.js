...
export default function OrdersPage() {
  ...
  const [size, setSize] = useState(productOptions[0].sizes[0]);
  const [paper, setPaper] = useState(productOptions[0].papers[0]);
  const [finish, setFinish] = useState(productOptions[0].finishes[0]);
  const [sides, setSides] = useState(productOptions[0].sides[0]);
  ...
  useEffect(() => {
    ...
    const matchedProduct = productOptions.find(
      (item) => item.name === productParam
    );

    if (!matchedProduct) return;

    setSelectedProduct(matchedProduct.name);
    setSize(matchedProduct.sizes[0]);
    setPaper(matchedProduct.papers[0]);
    setFinish(matchedProduct.finishes[0]);
    setSides(matchedProduct.sides[0]);
    setQuantity(matchedProduct.quantities[0]);
  }, []);
