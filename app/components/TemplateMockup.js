...
export default function TemplateMockup({ template, brandName="Envision Direct", headline, sub, cta, contact="555-123-4567 • envisiondirect.net", logoText="ED", accent, bg }) {
  const finalAccent = accent || template.accent;
  const finalBg = bg || template.bg;
  const finalHeadline = headline || template.headline;
  const finalSub = sub || template.sub;
      ...
      <div style={{width:widthFor(template.product),aspectRatio:aspectFor(template.product),borderRadius:28,background:finalBg,padding:22,border:"1px solid rgba(15,23,42,.08)",boxShadow:"0 30px 70px rgba(15,23,42,.12)",display:"grid",gridTemplateRows:"auto 1fr auto",gap:18,color:dark?"#ffffff":"#0f172a",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,position:"relative"}}>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            <div style={{width:54,height:54,borderRadius:18,background:finalAccent,color:"#fff",display:"grid",placeItems:"center",fontWeight:900}}>{logoText}</div>
            <div><div style={{fontSize:20,fontWeight:900}}>{brandName}</div><div style={{fontSize:12,opacity:.72}}>{template.industry}</div></div>
          </div>
        ...
        <div style={{display:"grid",gap:16,alignContent:"start",position:"relative"}}>
          ...
          <div style={{fontSize:16,opacity:.8,maxWidth:"85%",lineHeight:1.45}}>{finalSub}</div>
        </div>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,borderTop:"1px solid rgba(15,23,42,.08)",paddingTop:14}}>
